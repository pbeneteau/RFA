...
| [05-concurrency](05-concurrency/REPORT.md) | 2026-08-25 | How does one agent, and a fleet of them, safely run many tasks at once when runs share an identity, a workspace, a memory store, SDK sessions and a task board? (6-dimension sweep, each dimension adversarially verified, two live SDK probes, 45 papers, then a completeness pass) | The run, never the turn, is the unit of parallelism; the turn lock is permanent architecture |
