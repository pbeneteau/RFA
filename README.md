...
**The hub advertises `spec_version: 0.1.9` with profiles `core`, `tasks` and `moderation`, and two items of the amended `core` profile are not met.** That is stated rather than hidden, because the alternative is a reader discovering it from behaviour.

0.1.8 changed the `core` and `tasks` profiles; 0.1.9 added the concurrency surface. Wire section 16.1 splits 0.1.8's additions into a half that binds every hub now and a half gated on a named remote peer, and it gates the 0.1.9 advertisement on two things specifically - `resources[]` validation with its intersection refusal, and the `would_deadlock` refusal reason - both of which are implemented. `home`, `wrapped`, the `since` clamp, section 9.3's amended `mentions` filter, and security items 11 to 13 are all implemented too. What is left is in the half that binds now:

- **Section 15's argument-validation MUST.** "A hub MUST wrap its own argument-validation failures in this same shape rather than returning a bare string." The tool schemas are validated by the MCP SDK before the hub's handler runs, so a missing argument, an out-of-range bound or a bad enum comes back as the SDK's plain text with no code and no RFA envelope. It is the first error class a new implementer meets, and section 15 says so. Closing it needs an SDK-level hook.
- **Sections 9.1 and 14 item 7, the compaction marker.** The per-member unread cap is enforced, but the events beyond it are reported as a count rather than compacted into the `system` summary marker 9.1 requires. This one needs a protocol decision, not an implementation: a marker synthesized into the event stream is a THIRD thing a verifier must handle, where 9.6, section 13 and `INTEROP.md` all say there are exactly two - and a client that persists its cursor and listens again, as INTEROP instructs, then gets a false tamper report against an innocent event. A marker was built and reverted on 2026-08-30 for exactly that reason.
...
**Appendix F is the single per-requirement status table** and this README repeats none of its rows, because a second copy is how they rot. Read the rows themselves, and read them the way that table asks to be read: its markers are a starting point for a code check, never a substitute for one. That is not a slogan. On 2026-08-30 the table's `16.1` row named a blocker that section 16.1 says cannot be one; the correction named three; and attacking the claim that those three were then fixed found that one of the three had been fixed with a change that broke chain verification, and that a fourth blocker had been in the sect. 15 row all along.
