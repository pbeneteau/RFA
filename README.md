...
**The hub advertises `spec_version: 0.1.9` with profiles `core`, `tasks` and `moderation`, and one item of the amended `core` profile is not implemented.** That is stated rather than hidden, because the alternative is a reader discovering it from behaviour.

0.1.8 changed the `core` and `tasks` profiles; 0.1.9 added the concurrency surface. Wire section 16.1 splits 0.1.8's additions into a half that binds every hub now and a half gated on a named remote peer, and it gates the 0.1.9 advertisement on two things specifically - `resources[]` validation with its intersection refusal, and the `would_deadlock` refusal reason - both of which are implemented. The gap is elsewhere, in the half that binds now: **9.3's amended `mentions` filter**. The hub still matches a system event against an enumerated set of `refs` keys (`asker`, `askers`, `member`, and a `message_id` the caller sent) rather than against the general rule 0.1.8 introduced, which is any member id under any key. Three of the six keys section 9.3 names as already in use - `owner`, `author`, `requester` - are not matched, so a system event referencing a member under one of them is not delivered to that member under `wait_for: "mentions"`. `wait_for: "all"` is unaffected. Section 16.1 gives the reference implementation until hub release 0.7.0 to meet the amended core, and this is 0.6.4, so the profile does not yet *bind* here; the advertisement is what runs ahead of it.

**Appendix F is the single per-requirement status table** and this README repeats none of its rows, because a second copy is how they rot. Read the rows themselves, and read them the way that table asks to be read: its markers are a starting point for a code check, never a substitute for one.
