...
**The hub advertises `spec_version: 0.1.9` with profiles `core`, `tasks` and `moderation`, and as of 2026-08-30 it meets the amended `core` profile.** It is stated here because for most of this project's life it did not, and the README said so.

0.1.8 changed the `core` and `tasks` profiles; 0.1.9 added the concurrency surface. Wire section 16.1 splits 0.1.8's additions into a half that binds every hub now and a half gated on a named remote peer, and it gates the 0.1.9 advertisement on `resources[]` validation with its intersection refusal and the `would_deadlock` refusal reason. Section 16.1 gives the reference implementation until hub release 0.7.0 to meet the amended core; this is 0.6.4, so it is met ahead of that rather than at it. Half B (admission records, invites, guest transport authentication) remains unimplemented by design and is gated on a named counterparty, which section 16.1 says a guestless hub must not be judged on.

Two of the last items are worth knowing about, because both changed how the hub answers:

- **Argument validation now returns an RFA error.** The MCP SDK validates a tool's schema before the hub's handler runs, so a missing field used to come back as `Input validation error: ...` - prose with no code. The hub now hands the SDK a proxy that publishes a byte-identical schema and defers enforcement, so a violation is a normal `bad_request` in the same envelope as every other refusal. Section 15 exists for exactly this, and it is the first error class a new implementer meets.
- **The unread cap reports what it dropped.** Beyond the per-member cap, `room_listen` returns `compaction: {dropped, from_seq, to_seq, cap}` beside `events`. It is deliberately NOT an event: 9.6 and section 13 both say a verifier handles exactly two deviations, and a marker synthesized into the stream makes a client that persists its cursor report a false tamper against an innocent event. That was built, measured and reverted before the spec was amended to name the delivery.

**Appendix F is the single per-requirement status table** and this README repeats none of its rows, because a second copy is how they rot. Read the rows themselves, and read them the way that table asks to be read: its markers are a starting point for a code check, never a substitute for one. That is not a slogan. The `16.1` row first blamed a blocker that section 16.1 says cannot be one; correcting it named three; and attacking the claim that those three were fixed found that one fix was a regression and that a fourth blocker had been in the section 15 row all along.
