...
**The trigger, corrected, and the walkthrough describe-first (amendment, 2026-08-30).** Two changes from the Goodvest dogfood (findings F5, F8, F9) and the LangChain recon:

1. **A bare name on a terminal enters the walkthrough with the name pre-filled.** `rfa agent new <name>` used to mean headless-with-defaults, which is the opposite of the natural reading: the user types the name BECAUSE the command needs one, and got an unspecified generic agent with no questions asked. Headless scaffolding now requires a configuration flag (`--kind`, `--room`, `--knowledge`, `--model`, `--server`, `--command`, `--builtin`, `--tool`, `--mode`, `--dry-run`), `--yes`, or a pipe - and its success line names the model, the mode and the budgets it chose, the three things that cost money and act, read back from the definition as written.
2. **When a model credential exists, the walkthrough opens describe-first.** One free-text question ("describe the agent: what should it do, from what, for whom"); one bounded model call (`src/cli/draftpack.ts`) drafts the complete pack - kind, name, description, model, capability, budgets, a knowledge folder the description itself named (never one invented), and the system prompt - and the wizard renders the COMPLETE proposed agent.md beside the review with every question pre-answered, esc walking back into any screen to edit. Nothing is written until the review is approved, through the same `scaffoldPack`. The draft call is a model-call site under RFA-0.9 sect. 6: it is in `src/querysites.ts` with `tools: []`, a fresh isolated cwd per call, connector suppression, and its cost rendered on the review (`drafted by <model> · $<cost>`). The drafted output is never trusted because a model wrote it: every field is coerced against the schema's own grammar (a miss falls back to the kind's default WITH a note the review shows), the platform appends the data-not-instructions rule to every drafted prompt itself, and the composed file passes `parseAgentMd` - the supervisor's schema - before the wizard shows it. No credential, or a blank description, is the plain walkthrough unchanged. A drafted tool user always starts in `ask` mode and still answers the server screen itself; a draft never invents an MCP command.
3. **Creation ends at an offered first ask, not a silent success line**, when the pack is bound to a recorded room and the hub and supervisor are running: the wizard waits for the resident's own `state/member.json` and heartbeat (the running system's record), then asks by capability through the same `askInRoom` the dashboard uses, and renders the answer with its cost and run id. Blank skips; a pack with nothing running gets the done screen naming `rfa up`.
...
rfa agent new [<name>] [--kind spec-expert|answerer|tool] [--room <alias|handle>] [--knowledge <dir>] [--model haiku|sonnet|opus]
                                       # on a terminal: the walkthrough, describe-first with a model credential, a bare
                                       # name pre-filled (13.7 amendment 2026-08-30); headless needs a flag, --yes or a pipe
