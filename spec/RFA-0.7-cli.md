...
| `rfa agent edit <name>` | Opens agent.md in `$EDITOR`, validates on save, tells you the supervisor will drain and respawn | new |
...
| `rfa evals run [--judged] [--update-baseline]` / `ls` / `promote <room> (--conversation\|--task) --id` / `label --prepare\|--apply` / `parity [--capture]` | The eval gate, the flywheel, the labelling sitting, and the parity gate reading `evals/parity.json` | `src/evals/runner.ts`, `scripts/promote-case.ts`, `scripts/label.ts`, `dogfood/parity.ts` |
...
rfa agent start|stop|restart <name> | edit <name> | retire <name> [--dry-run] [--timeout]
...
rfa evals run [--judged] [--update-baseline] | ls | promote <room> (--conversation|--task) --id [--failure-mode] | label --prepare|--apply | parity [--capture]
