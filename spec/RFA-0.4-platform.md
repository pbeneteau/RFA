...
| `bypass` | routed through `canUseTool`, allowed without a card | consulted; undeclared tools still denied | `default` (amended 2026-08-24; see below) |
...
**Amended (2026-08-24): `bypass` no longer uses the SDK's `bypassPermissions`.** That permission mode auto-approves EVERY reachable tool before `canUseTool` is consulted — and a resident authenticated through the operator's claude.ai login can reach the account's own connectors, plus the SDK's built-in tools. Found live: a `linear-agent` whose pack declares one gated document tool created real Linear projects and issues through the operator's personal Linear connector (`mcp__claude_ai_Linear__save_project`/`save_issue`, recorded in the session transcript), with the deny-by-default `canUseTool` never asked; the reports were accurate and the evidence was accepted, which is exactly why the hole mattered. `bypass` now keeps the default permission flow: the pack's acting tools stay OUT of `allowedTools` so they route through `canUseTool`, where the mode answers **allow without a card**; every tool the pack did not declare still dies at the same door as in `ask` ("not allowed for this pack"). What `bypass` means is therefore exactly its sentence: the pack's own acting tools skip the human — never "every tool this login can reach skips every gate".

**A resident's tool surface is its declaration, and three layers enforce it (amendment, 2026-08-25).** A mode decides what happens to the pack's *acting* tools; it never widens the set. What the resident may reach at all is fixed by three separate mechanisms, because one of them alone is not a fence:

1. **MCP tools** — the pack's `tools.allow` minus its acting tools go in the SDK's `allowedTools`; everything else falls through to `canUseTool`, which denies by name ("tool X is not allowed for this pack").
2. **Built-in (harness-internal) tools** — `canUseTool` is **never consulted** for these, so the callback fences nothing here. The resident passes the SDK's `tools` option (its base built-in set) as exactly the built-ins the pack declared, so every other one is absent from the model's context rather than denied in it. A pack that declares none gets none, and a built-in a future SDK adds is excluded by construction. Found live 2026-08-25: a resident ran `ToolSearch`, enumerated the operator's other Claude Code sessions with `ListAgents`, and called `SendMessage` at one, none declared, none refused.
3. **The operator's account connectors** — a resident authenticates with the operator's login, and a claude.ai account's MCP connectors are auto-fetched into the session. They ride the login rather than a settings file, so `settingSources: []` does not cover them; the resident passes `settings: { disableClaudeAiConnectors: true }` inline. Found live 2026-08-24: a pack holding one gated document tool created real Linear projects and issues through the account's Linear connector.

The same principle governs the platform's own voice: `mcp__rfa__ask` is registered on the rfa server only for a pack that declares it, because a tool a pack cannot call has no business in its prompt.
