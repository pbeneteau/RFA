...
| 9.1, 14 item 7 | The unread cap's `system` summary marker | **PENDING, and it needs a wire decision rather than an implementation** (documented 2026-08-30, after a marker was built and REVERTED the same day). The cap is enforced at its default of 200 and the overflow is reported as the numeric `compacted` field on the `room_listen` result, so a client is told how many events it lost and nothing about them. A synthesized marker in the event stream is the obvious fix and it CONFLICTS with three published statements: 9.6 says a verifier of received events "now has TWO things to handle, not one", sect. 13 says it needs no field surgery "in **two** cases and exactly two", and INTEROP.md repeats "in two ways, and only two". A marker is a third and nothing announces it. Measured on the reverted attempt: placed FIRST in one listen result it verifies, because `verifyChain` skips a leading unchained prefix - but a client that persists its cursor and listens again, which INTEROP sect. 4.1 instructs, accumulates it MID-stream and gets a second divergence carrying a FALSE tamper report against an innocent event (1 divergence became 2). Closing this needs either a fourth verifier rule or delivery outside the events array; both are protocol revisions. `test/leases.test.ts` pins the current honest behaviour and the reason |
...
| 16.1 | The amended core profile as a whole | **PENDING**, and this row has now named the wrong blocker twice - once until 2026-08-30, and once when a same-day claim that it was MET was defeated by attacking it. It said the cause was 4.3's parked admission half; section 16.1's own table puts admission in **half B** and says in terms that "a hub with no guests MUST NOT be judged non-conformant for omitting half B", so that was never the reason. The blockers are in half A, which binds every hub now. 9.3, 14 item 11's application and 14 item 12 all shipped on 2026-08-30, and TWO remain, both verified against the code: **sect. 15's argument-validation MUST** ("a hub MUST wrap its own argument-validation failures in this same shape rather than returning a bare string") - the tool schemas are validated by the MCP SDK before the handler runs, so a missing argument, an out-of-range bound or a bad enum still returns the SDK's plain text with no code and no envelope, which the sect. 15 row below has recorded as PARTIAL all along and which sits INSIDE core because core's list names "errors"; and **9.1's compaction marker**, which needs a wire decision rather than an implementation (its row explains why an in-stream marker breaks chain verification for a conforming peer). It binds from hub release 0.7.0 and this is 0.6.4, so the grace covers BINDING; the hub nevertheless advertises `core`, which runs ahead of it |
