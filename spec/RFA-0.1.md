...
| 9.1, 14 item 7 | The unread cap's `system` summary marker | **PARTIAL, first documented 2026-08-30.** The per-member unread cap exists and is enforced at its default of 200, but 9.1 says the events beyond it are "compacted into a `system` summary marker" and no such event is ever appended: the overflow is sliced off and reported as a numeric `compacted` field on the `room_listen` result. A client is therefore told HOW MANY events it lost and nothing about them, and the log carries no record that a compaction happened |
...
| 16.1 | The amended core profile as a whole | **PENDING**, and this row named the wrong blocker until 2026-08-30. It said the cause was 4.3's parked admission half; section 16.1's own table puts admission in **half B** and says in terms that "a hub with no guests MUST NOT be judged non-conformant for omitting half B", so that was never the reason. The blockers are in half A, which binds every hub now, and after 9.3 shipped they are three, each verified against the code rather than against this table: **14 item 11's APPLICATION** (the character classes are complete, but peer text reaching a model outside the 9.6 boundary is not neutralized - `refusal.detail` and peer skill ids, rows below); **14 item 12's first half** (numeric self-reports are not rendered as self-reported); and **14 item 7's compaction marker** (the unread cap drops events without appending the `system` summary marker 9.1 requires). Item 12's second half, the one that matters for safety, holds. It binds from hub release 0.7.0 and this is 0.6.4, so the grace covers BINDING; the hub nevertheless advertises `core`, which runs ahead of it |
