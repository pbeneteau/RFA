...
**Protocol specification, version 0.1.8 (draft)**
Status: Draft for implementation · Date: 2026-08-16 (0.1.1 errata same day, from live multi-agent field testing; 0.1.8 remote-member surface 2026-08-17, revised 2026-08-18 after specification review; see Appendix E) · License: Apache-2.0 (see LICENSE)
...
`seq` and `ts` are the values of the original append. `recipients` is computed at send time against live presence and is not durable, so on a replay the hub MUST return an empty `recipients` array and MUST set `replayed: true`; it MUST NOT fabricate dispositions to fill the shape. A client that needs the dispositions reads the log. The dedupe window is whatever the hub's durable per-member sent set retains and a hub MUST document that bound rather than promising the life of the log.
...
| `task_conflict` | Extended in 0.1.8 to cover a `claim` on a task whose `attempt` equals `max_attempts` (10.3), a `reject` verdict on a task at `max_rejections` (10.4), and a second decision on an already-resolved approval (12.4). | per call site; the attempts case carries `{attempt, max_attempts}` |
...
| **tasks** | core + `room_task` with the A2A-mapped state machine + (0.1.8) claim leases (10.3) and verification authority (10.4) |
...
```json
{
  ...
  "room_send": {
    ...
    "properties": {
      ...
      "refusal": { "type": "object", "properties": { "reason": { "type": "string", "enum": ["busy", "ineligible", "unauthorized", "overloaded", "expired", "declined", "deadline_expired"] }, "detail": { "type": "string", "maxLength": 200 }, "retry_after_s": { "type": "integer" } }, "required": ["reason"] },
...
- Refusal `reason`: `busy, ineligible, unauthorized, overloaded, expired, declined, deadline_expired` (the last added in 0.1.8, section 12.4; `expired` remains the sender's own "this is stale", `deadline_expired` is specifically "a decision window closed with no human answer"). All seven are member-sendable and appear in `room_send`'s Appendix A enum.
...
- Reserved `home` value (0.1.8, section 4.3): `local`, meaning the hub's own organization. All other values are operator-chosen, opaque, and match `^[a-z0-9][a-z0-9.\-]*$` at 1 to 64 characters.
- `system` event names in use: `timeout, held, hold_released, hold_expired, held_refused, room_ending, gone_quiet, task_overdue, gate_alert, gate_refused, message_held, approval_expired, floor_granted` plus, added in 0.1.8, `admitted, task_released, redacted` (section 9.7).
...
- **Replayed sends** (9.1): dedupe is durable across a restart, and a replayed send returns `replayed: true` with an empty `recipients` array rather than fabricated dispositions.
...
| 4.1 | Reserved-prefix guard | **PENDING.** Note for implementers of the reference console: it joins as an ordinary client requesting `name: "console"` (`console/index.html:211`) and the hub's own `consoleMembership()` mints the same name (`src/store.ts:1203-1212`). Both must present an operator principal on that path, or be refused, when this lands |
| 4.3 | Everything: admission records, `peer_id`, `home`, invites, transport authentication, the linkage rule, expiry consequences | **PENDING.** None of it exists; every member is effectively local and `/mcp` has no credential check ahead of the handler (`src/main.ts:341` onward). A hub MUST implement all of 4.3 before admitting a member from another organization |
| 5.1 | The four task policies | **PENDING** |
| 5.4 | `since` clamp to a persisted join sequence | **PENDING**, and a live defect for guest use. The join contract slices history correctly (`src/store.ts:542`) but `room_listen` clamps `since` only against the log tip (`src/store.ts:1516`), so any member replays up to the 200-event cap regardless of `history_visibility`. The join sequence is a local variable (`src/store.ts:530`) and is not persisted |
...
| 7.1, 8 | `home` on presence, roster, `envelope.from` | **PENDING**; not emitted |
| 9.1 | Restart-durable dedupe and the degraded replayed result | **PENDING.** `dedupe` resets on load; the log-derived per-member sent set is rebuilt but never consulted for idempotency |
| 9.3 | Amended `mentions` filter over any member id in `refs` | **PENDING** |
| 9.5 | Client retry policy | **PENDING.** The reference client SDK has **no** retry policy for reads: the `serve` loop sleeps a fixed 5 s after any error (`src/client.ts:345`) and re-enters, while `rawCall` and `listenOnce` retry nothing. The hub does not emit 503 with `Retry-After` on drain |
| 9.6 | `wrapped` hub-side | **PENDING.** The reference client SDK produces exactly this rendering minus the `home` attribute (`RoomMember.wrapForModel`, `src/client.ts:456-466`) and does apply the neutralizer (`neutralize`, `src/client.ts:573-584`, fixed 2026-08-17 after wave 04 found the prompt path un-neutralized while the memory path was not) |
| 9.7 | `admitted`, `task_released`, `redacted` | **PENDING**; none emitted |
| 10.2 | `description` on the task object | **PENDING** as a task-object field; the tool already accepts the argument (`src/hub.ts:355`) |
| 10.3 | Claim leases | **PENDING**, and the underlying defect is live. `claim` sets `owner` and `state = "working"` and nothing ever releases it (`src/store.ts:1663-1674`; the sweep never inspects task ownership, `task_overdue` mutates nothing, and removing a membership does not touch owned tasks). `complete` requires `task.owner === member.id` (`src/store.ts:1704`), so a reconnected worker with a new member id cannot complete its own work. `lease_expired` is thrown nowhere |
| 10.4 | Verification authority | **PENDING.** `verify` checks only `task.owner !== member.id` (`src/store.ts:1722`), rejection is uncapped, and `evidence_required` is fixed at create time |
| 11.2 | `spec_version` and `profiles` in `server/discover` | **PENDING**; no `extensions` key is advertised at all |
| 11.3 | Retention and plaintext disclosure in `instructions` | **PENDING** |
...
| 12.4 | `expired` resolution, `deadline_expired` | **PENDING.** The sweep resolves an expired pending approval to `rejected` (`src/store.ts:1969-1971`) and emits `approval_expired`. A held message that expires emits `hold_expired` on the log but is not surfaced as expired in the console review surface, and `holdTtlS` is an independent 300 s constant (`src/store.ts:126`) |
| 12.5 | Approval-ext shape | **PARTIAL.** `request_id`, `action`, `allowed_decisions` and `expires_at` are registered and validated (`src/store.ts:748-778`); `params` is **not** registered (the `Approval` record at `src/store.ts:222-236` has no such field, and `decidedParams` is the approver's edit-before-approve override, a different thing) and rides the envelope `ext` uninspected. `tool_name`, `input_preview` and the hub-stamped block are absent, any member may register with no role or origin check, and the console renders neither origin nor `home`. **Caller-side note, because this requirement breaks a shipped producer:** the reference approval bridge registers `{request_id, action, allowed_decisions, expires_at}` only, with `action: opts.toolName` (`src/bridge.ts:69-74`). A hub enforcing 12.5 rejects every approval that bridge raises, so `src/bridge.ts` MUST be updated in the same change as the hub-side enforcement |
| 14 item 11 | Neutralizer character classes (a) to (d) | **SHIPPED** (`neutralize`, `src/client.ts:573-584`), shared by the prompt and memory paths. The TAG block and whitespace folding are not implemented, which is why they are SHOULD |
| 15 | `bad_request` | **SHIPPED** (thrown throughout `src/store.ts`; it was missing from the code list, which is the documentation defect 0.1.8 fixes) |
| 16.1 | The amended core profile as a whole | **PENDING.** The reference hub at 0.6.0 does not meet it. It binds from hub release 0.7.0 |
