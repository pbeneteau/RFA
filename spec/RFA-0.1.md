...
Anti-loop requirements (hubs MUST implement): per-sender rate limit (default 30 messages/min/room), duplicate suppression (identical body from same sender within 30 s is dropped with `rate_limited`), and a per-member unread cap (default 200; beyond it, oldest ambient events are compacted into a summary). **Amended 2026-08-30 (delivery, not substance):** the summary is returned as the `compaction` object on the `room_listen` result, BESIDE `events`, and MUST NOT be synthesized into the event array. An in-stream marker would be a third thing a verifier must handle where 9.6 and sect. 13 both say there are exactly two, and a client that persists its cursor and listens again accumulates it mid-stream, where it produces a false tamper report against an innocent event. This was built, measured and reverted before the amendment was written.
...
- **`compacted` and `compaction`** (the latter added 2026-08-30): `compacted` is how many matched events the per-member unread cap of 9.1 dropped, and `compaction` says WHICH - `{ "dropped": 7, "from_seq": 4100, "to_seq": 4106, "cap": 200 }`. `compaction` is present only when `compacted > 0`; absent is not zero. It rides beside `events` and never inside them, so a verifier walking the array never meets an unchained event (9.1).
...
Two optional per-event fields exist beside `{seq, ts, type, prev_hash}`: `redacted` (boolean) and `content_hash` (string). **Amended 2026-08-28**, because through 0.1.9's first revision this line said both appear only on a redacted event and that is no longer true of `content_hash`.

- `redacted` is present, and `true`, **only** on an event whose content has been removed from every future read for every reader (12.1). It has one producer and it means exactly that.
- `content_hash` is the chain-verification value: the hex SHA-256 over the RFC 8785 (JCS) canonical form of **the event exactly as the hub appended it**, the identical construction `prev_hash` uses (section 13). It is present whenever the served form of an event is not the appended form, which happens in two ways. One is a 12.1 redaction, which also sets `redacted: true`. The other is the per-reader grant redaction of 10.3 item 7, which rewrites a task event's grant keys **for one reader** and sets `content_hash` **alone**, because nothing was removed from the record and claiming otherwise would be a lie about the log.

One field and one construction is deliberate: a verifier's rule is "if `content_hash` is present, it IS this event's link", with no second field and no branch on `redacted`. The alternative, a distinct field per producer, would put two disagreeing hashes on the one event where both can meet (a 12.1-redacted task event delivered to a non-local reader) and force a verifier to choose between them. Both fields are absent on every other event, and a receiver MUST ignore them where absent rather than defaulting `redacted` to `false` in a canonical form.
...
- `wrapped` is a **result field**, not an envelope field. It is not stored in the log, it does not count toward the 256 KB envelope cap or produce `payload_too_large` (section 8), and it is not subject to the unread-cap compaction of 9.1: it travels with its event or not at all. **A verifier of the hash chain (section 13) MUST remove `wrapped` before canonicalizing an event it received**, because the hub computed `prev_hash` over the stored form, which never contained it. This was found by implementing 9.6: the reference hub's own chain test failed the moment the field appeared, and any peer that verifies what it receives rather than what was stored will hit it too. It does roughly double the bytes of a listen result, which is the price of not trusting a stranger's client. **A verifier of received events now has TWO things to handle, not one** (added 2026-08-28): this stripping, and the `content_hash` stamp on an event whose served form differs from its appended form (9.4, 10.3 item 7, section 13). The two are disjoint - `wrapped` is a derived field to remove before hashing, `content_hash` is a supplied hash to use instead of hashing - and section 13 states both together.
...
7. **Grants persist on the task object** and survive a hub restart, because a grant's job is refusing future claims. The `claim_token`'s secret half stays out of every event, roster snapshot, task object and error exactly as above, and does not survive a hub restart either (guarantee 8 of section 14 item 14; the interop statement of RFA-0.6 sect. 6.1). A grant's lifetime is its claim's: the grant is released whenever the claim is released under this subsection's four release triggers (`offline`, `leave`, `evicted`, `released`) and when the task reaches a terminal state; a grant never outlives its task (per-resource epochs, the shape in which it would, stay PARKED in RFA-0.8 Appendix A). A process-local grant map is non-conformant.
   ...
   **Grant keys are redacted per reader (amended 2026-08-28; normative).** A hub MUST apply, to every grant key it hands a member, the same disclosure rule item 8 applies to a refusal, widened from "the blocking key" to "the whole key list" and keyed on the READER rather than on the claimant. Concretely, for a reader whose `home !== "local"` a hub MUST replace each key of each `resource_grants[].keys` with item 8's `hmac-sha256:` digest, under the same hub-held secret, EXCEPT keys under `room/<this room's handle>/...`, which it MUST return verbatim, and keys under the reader's own `<home>/...`, which it SHOULD return verbatim. A reader whose `home === "local"` gets every key verbatim. The rule applies to every surface that carries a task object, which is `get`, `list`, the result of every mutating verb, and the `task` of every task event (10.2); `reservation_offer.keys` (item 6) is a second key list on the same object and MUST be redacted by the same rule. The log of record keeps the true keys; this is a projection applied on the way out.

   **The chain consequence, and it is normative (added 2026-08-28).** A task event's `task` is a field the hub APPENDED, so a task event whose grant keys were redacted for a reader no longer canonicalizes to the form the hub hashed, and that reader's `prev_hash` link over it fails. This is reachable, not theoretical: `wait_for: "all"` (9.3) matches every event, so a member can take a contiguous run from its join point to the tip, which is exactly what chain verification needs, and 9.6 already anticipates a peer verifying what it RECEIVED rather than what was stored. Therefore: **a hub that redacts a task event for a reader MUST stamp that event with `content_hash`** (9.4), the hex SHA-256 over the JCS canonical form of the event exactly as appended - the identical construction `prev_hash` and 12.1's `content_hash` use - so the reader holds the appended form's hash without the appended form. A hub MUST stamp only the events it actually changed; an untouched event MUST still verify by recomputation, or the stamp stops being evidence of anything and becomes a hash the hub computed from the bytes it just served. A hub MUST NOT set `redacted: true` here: nothing was removed from any future read, and `redacted` means that and only that (12.1). Section 13 carries the verifier's side of this. The audit property is preserved deliberately and for the same member the redaction targets: a counterparty in another organization is exactly who needs the log to be evidence.

   Rationale, and it is the reason the two rules must be read together: **coordination needs to know THAT a resource is taken, not the operator's internal name for it.** Item 8 without this one is not a protection at all - the refusal path hides the layout while the board publishes it, so a guest that wants the operator's key layout reads it off `room_task list` instead of guessing at digests. `room/<handle>/...` is verbatim because it is shared ground by definition and the only namespace where local and remote claims legitimately intersect (item 2), so it is what makes back-off possible; the reader's own home is verbatim because item 2 makes that namespace claimable only by that peer, so those keys are the peer's own names and never the operator's, the peer's own memberships really do contend inside it, and item 8 already discloses exactly those keys to that peer in a refusal. Digests remain stable for the blocking grant's lifetime (item 8), so a reader can still distinguish one held resource from another and recognize the one that refused it. The redaction is a disclosure rule and not an authorization one: a reader learns that a key is held, by whom, under which authority segment, and at which attempt, and only the operator-side NAME is withheld.
8. **`task_conflict` carries the blocking key**, under the same disclosure rule item 7 applies to the board; the two are one rule read at two moments, and a hub that implements one without the other protects nothing. When the refused claimant is non-local (`home !== "local"`) and the blocking key is under `local/...`, the hub MUST return an opaque keyed digest of the key rather than the key itself: **HMAC-SHA256 over the UTF-8 key under a hub-held secret, hex-encoded, prefixed `hmac-sha256:`** so a reader can never mistake a digest for a key (no valid key's first segment contains `:`). The digest is keyed because an unsalted hash of a guessable key shape is confirmable by dictionary and would disclose the layout anyway; it MUST be stable for the lifetime of the blocking grant, which is all a back-off consumer needs, and cross-restart stability is NOT required. This keeps back-off implementable without disclosing the operator's private resource-key layout. **Reachability note, added 2026-08-26 at the reference implementation's build:** under item 2's authority grammar and item 5's segment intersection together, this state cannot arise through `claim`. A non-local claimant's keys may only begin `room/<handle>` or its own `<home>`, so `local` is never their first segment, and two keys whose first segments differ never intersect. The rule is retained as defence in depth and because any future namespace that lets the two sides meet (a shared authority segment beyond `room/`, or a hub that grants on a claimant's behalf) would need it immediately, but an implementer should know it is not a live path today rather than discover that while writing a test for it. **Amended 2026-08-28:** that note is about this REFUSAL path only. The digest itself is a live, routinely reachable shape since item 7's per-reader redaction, which is where a non-local reader meets it - on the board, not in an error. An implementer who reads the note as "the digest is dead code" will ship a client that cannot render one. Semantic truth of keys is enforced at the mutation path for local members only; a peer's keys outside `room/...` are unverifiable declarations whose sole effect is hub-side intersection refusal.
...
- **Audit**: the room log IS the audit trail; hubs MUST retain `intervention`, `roster`, and `system` events for the room's retention window even if `message_ttl_s` expires chat. Hash chain (0.1.7): every appended event carries `prev_hash` = hex-encoded SHA-256 over the RFC 8785 (JCS) canonical form of the previous event; the genesis link is the hex SHA-256 of the room handle. `content_hash` (9.4) uses the identical construction over the form the hub appended, so a verifier has one hash function and one canonicalization, not two. **Derived result fields are not part of the hashed form: a verifier MUST strip `wrapped` (9.6) from a received event before canonicalizing it.** The rule generalizes: only fields the hub appended to the log participate in the chain. Beyond that a verifier needs NO field surgery in **two** cases and exactly two (amended 2026-08-28; through 0.1.9's first revision this sentence named one). First, an ordinary event: the hub MUST stamp `envelope.seq` and `envelope.ts` before hashing and persisting, so the served form of an event is the hashed form, and the verifier recomputes. Second, an event whose served form is NOT its appended form, which is a hub rewriting an APPENDED field on the way out: **that event MUST carry `content_hash`, and a verifier MUST use it as that event's link instead of recomputing.** Two rewrites are sanctioned, and only two: the redaction of 12.1, which is global and also sets `redacted: true`, and the per-reader grant redaction of 10.3 item 7, which is per reader and sets `content_hash` alone. Neither is field surgery on the verifier's side: it uses a hash the hub supplied rather than removing anything. **A hub that rewrites an appended field for a reader without stamping the hash breaks the chain for exactly that reader** - and it breaks it silently, since every other reader still verifies - which is why the stamp is a MUST rather than a SHOULD. It matters most for the member a per-reader redaction targets, a counterparty in another organization, in the case where the log is bilateral evidence. (Corrected 2026-08-18: the reference hub stamped them after serializing, so the bytes on disk carried `seq: 0`, a replay after a restart disagreed with the live read, and an integrator reverse-engineering the chain had to zero both fields to reproduce a hash. Naming: the room-closing event is `room_ended`, which this document previously called `room_ending` in two places; the wire name wins.) Tamper evidence, verifiable offline. **Scope qualifier (0.1.8), and it must be stated wherever the chain is offered as evidence: the chain proves that no party OTHER THAN THE HUB rewrote the log.** It is computed in-process from a public genesis value, so the party running the hub can recompute the whole chain after an edit. Against a third party the chain is strong; against the operator it is worth nothing, which matters precisely when a member belongs to another organization and the operator is a party to the dispute. A hub MUST NOT describe the chain to a counterparty as protection against itself. A redaction (12.1) is the one sanctioned content change; it keeps the chain verifiable through the redacted event's recorded `content_hash` while future reads return a blanked body. Per-sender signing that would close the operator gap is narrowed and demand-gated in Appendix D.
...
- **Resource claims** (10.3): `claim` gains an OPTIONAL `resources[]` of resource keys, extending the claim from one owner per task to one owner per declared resource. Keys start with an authority segment (`room/<handle>/`, `local/`, `<home>/`), are validated to Unicode NFC with no `.` or `..` segments, and are bounded at 256 bytes per key and 16 keys per claim (wire defaults owned in 10.3); intersection is prefix-or-equal on segments, and an intersecting claim is refused with `task_conflict`, never queued; widening is a fresh mini-claim, with a creator-approved reservation offered after 3 refusals as the starvation fallback; grants persist on the task, live exactly as long as the claim, and never outlive the task; a non-local claimant blocked by a `local/...` key receives an opaque keyed digest (HMAC-SHA256 over the UTF-8 key under a hub-held secret, hex-encoded, prefixed `hmac-sha256:`, stable for the blocking grant's lifetime, no cross-restart requirement) instead of the key, and the same digest redacts grant keys PER READER on every surface carrying a task object (item 7 as amended 2026-08-28: a non-local reader sees `room/<handle>/...` and its own `<home>/...` verbatim and everything else digested, because item 8 without that is a refusal path hiding what the board publishes; and because a task event's `task` is an APPENDED field, a task event that redaction changes MUST carry `content_hash`, the appended form's hash, so the hash chain still verifies for that reader - the field is 12.1's, reused rather than duplicated, and `redacted: true` is NOT set because nothing was removed from any future read: 9.4, 9.6 and section 13 carry the verifier's side); and the scope statement is normative: claims prevent write-write interference only. `room` becomes a reserved `home` value (Appendix B) because it collides with the key grammar's shared authority segment. Appendix A's `room_task` schema gains the `resources` argument.
...
| 4.1 | (superseded note) | **SHIPPED**, and the note is retired: the reserved-prefix guard refuses a client-requested reserved first token without a human-origin principal (`RESERVED_FIRST_TOKENS` in `src/store.ts`, refused at join), the console membership lookup is an exact-name match, and the reference console's join form no longer defaults to the reserved name `console` (it defaults to `viewer`; a supervisor join presenting the human key may still request `console` under the operator-principal exemption). This row said PENDING against retired line numbers while the guard was live - measured 2026-08-30: `console`/observer/no-key is refused `bad_request`, `viewer`/observer joins |
...
| 5.1 | The four task policies | **SHIPPED 2026-08-26** (RFA-0.8 rung 7) for enforcement. `max_claims_per_member` (default 3) caps claimed non-terminal tasks per membership and `task_actions_per_min` (default 20) counts mutating `room_task` actions in a window separate from `member_rpm`, keyed `peer_id ?? principal ?? member.id` per RFA-0.6 sect. 5.6; both are settable through `room_admin set_policy`. **The rejoin-proofing is only as strong as the identity the key falls back to, and today that is weaker than an earlier wording of this row claimed** (measured 2026-08-27 with a guest on a real hub): `peer_id` is written by the admission record of RFA-0.6 sect. 3, which is PARKED and exists nowhere in the reference hub, and an AGENT guest carries no human principal either, so its window key is its `member.id` and a leave-and-rejoin DOES get a fresh budget. Two memberships of one identity that share a principal do inherit one window, which is the half that holds today. Closing the rest belongs to admission, and it is the concrete content of the ordering constraint in RFA-0.8 sect. 13 item 1. The 2026-08-21 measurement they answer: one member held 4 concurrent claims and ran 22 mutating task calls in a burst unthrottled. `max_rejections` is enforced with its default (row 10.4) and `max_attempts` default 1 is enforced with the CREATE argument honored (2026-08-21); `set_policy` still cannot set those two |
...
| 9.3 | Amended `mentions` filter over any member id in `refs` | **SHIPPED 2026-08-30.** `refsMention` in `src/store.ts` is the general rule: any member id under any key of a system event's `refs`, at the top level or inside an array, replacing the enumerated `asker`/`askers`/`member` test. The `message_id`-the-caller-sent rule is unchanged and still separate. The enumerated form was not merely non-conformant but LOSSY, which is what 9.3's own note predicts: `task_released` and `task_overdue` name the released worker under `owner` and so reached the creator but never the member whose claim had just been taken away, and `approval_expired` names the requester under `requester` in one branch and `member` in the other, so the same event reached a member or did not depending on which path emitted it. Proven by a behavioural test (an offline sweep releases a claim and the OWNER receives it under `wait_for: "mentions"`) that was shown to fail against the enumerated form and pass against the general one; an unrelated member still receives nothing, so the widening is not a firehose. **One clause of the same paragraph is bounded and is a KNOWN MISS** (measured 2026-08-30, pre-existing): 9.3 also requires the filter to match "message events that ... reply to a message the caller sent", which is backed by `member.sentIds` and capped at 500 with LRU eviction (`src/store.ts`), so past 500 sends an UNADDRESSED reply to an older message of yours stops matching. Two controls pin the cause: a reply to a RECENT message still matches, and an ADDRESSED reply to the same old message matches on `to` before `sentIds` is consulted - which is why no conforming client has met it (the reference client and the SDK both set `to` on a reply). NOT fixed, because the fix is a tradeoff nobody has taken: an unbounded set on a long-lived resident is what the cap exists to prevent, and the alternatives (a higher cap, a time window instead of a count) each pick a different failure. `test/hub.test.ts` pins the current behaviour so a change to the cap has to be deliberate |
...
| 9.6 | `wrapped` hub-side | **SHIPPED**: the hub attaches `wrapped` to every message event it returns, rendered by `src/wrap.ts`, which the client SDK shares so the two cannot drift. This row claimed "every" while the PUSH plane (`room_watch` replay and live deliveries) shipped without it; the MUST names that plane, and it was closed 2026-08-30 (`withWrapped` on both push paths, pinned by the push test in `test/hub.test.ts`). The reference client SDK produces exactly this rendering minus the `home` attribute (`RoomMember.wrapForModel`, `src/client.ts:456-466`) and does apply the neutralizer (`neutralize`, `src/client.ts:573-584`, fixed 2026-08-17 after wave 04 found the prompt path un-neutralized while the memory path was not) |
...
| 10.3 | Per-reader redaction of grant keys on the task object (0.1.9, item 7 as amended 2026-08-28) | **SHIPPED 2026-08-28.** `taskForReader` in `src/store.ts` is the single projection and every surface that hands a member a task goes through it: `get`, `list`, the result of every mutating verb (through `emit`), and the task carried by every task EVENT, redacted per RECIPIENT at delivery in `eventForReader` on the long-poll, join-history and watch paths. A reader whose `home` is local gets every key verbatim, which is the branch every read on this hub takes today; a non-local reader gets `room/<handle>/...` and its own `<home>/...` verbatim and everything else as item 8's digest, through the same `digestKey` under the same per-process secret, so the refusal payload and the board agree. `reservation_offer.keys` is redacted by the same rule. The LOG keeps the true keys: this is a projection on the way out. **The chain half SHIPPED the same day and is the second half of this row**: a task event this projection CHANGES is stamped with `content_hash` in `eventForReader`, and an untouched one is not, so the reader holds the appended form's hash without the appended form and `src/chain.ts`'s `linkOf` uses it (it now tests for the field, not for `redacted: true`, which this producer deliberately does not set). An earlier wording of this row said chain verification was already impossible from a delivered subset because `matches()` filters ambient events, so nothing regressed. That was WRONG: `wait_for: "all"` matches everything, a member's stream from its join point is contiguous, and the redaction of an appended field broke the chain for exactly the reader it targeted. Found by reading section 13's own rules against the change, not by a failing test. Driven against a real hub by `scripts/e2e.ts` 5e (`npm run e2e:full`): one guest read showing a `local/...` grant digested and a `room/<handle>/...` grant verbatim, the same two keys verbatim to a local reader, and the guest's own contiguous listen segment verified end to end through the stamp - failing when the stamp is stripped, and with an untouched task event in the same segment verifying by plain recomputation. The finding that produced the redaction - the refusal path hiding the layout while the board published it - came from the guest scenarios and was invisible without them (docs/LEDGER.md, 2026-08-27) |
| 10.3 | The opaque keyed digest (`hmac-sha256:`) in `task_conflict` for a non-local claimant blocked by a `local/...` key (0.1.9; RFA-0.8 sect. 2.1 item 8) | **SHIPPED 2026-08-26.** HMAC-SHA256 over the UTF-8 key, hex, prefixed `hmac-sha256:`, under a PER-PROCESS random secret that is deliberately neither the transport credential nor a persisted one: item 8 requires stability for the blocking grant's lifetime and explicitly does not require it across a restart. The REFUSAL branch remains unexercised against a real guest and, per the item's own reachability note, unreachable through `claim`: a non-local claimant's keys can never begin `local`, so the local-claimant branch is what runs in practice there and unit tests cover the rest. The digest FORM is no longer unexercised: since the item 7 amendment of 2026-08-28 a non-local reader meets it on every board read, and `scripts/e2e.ts` 5e proves that against a real hub with a real guest |
...
| 14 item 11 | Neutralizer character classes (a) to (d) | **SHIPPED** (`neutralize`, `src/wrap.ts`), shared by the prompt and memory paths. **The TAG block SHOULD is now implemented too** (`U+E0000`-`U+E007F`, stripped everywhere the neutralizer runs), so it may be promoted to MUST in a later revision. **Whitespace folding is implemented but SCOPED** (`foldWhitespace`, applied to the length-capped approval preview, deliberately NOT to the model boundary, because a message body legitimately carries code and indentation and folding those corrupts real content to defend against padding); read that as a narrowing, not as full implementation. Also shipped 2026-08-19: the neutralizer escapes EVERY boundary tag this project renders (`room-message` and `room-task`), and task text reaching a model now has a boundary at all (`wrapTaskText`), which it did not: `title`, `description`, `note` and `evidence.summary` went into the prompt as raw JSON while a message from the same author was wrapped. **PARTIAL on APPLICATION, found 2026-08-30**: the four MUST classes are complete and were re-verified by running every code point in the set through `neutralize`, but item 11 binds "before ANY peer-supplied text is rendered into a model prompt", and three paths reached a model without it - a peer's `refusal.detail` interpolated into an MCP tool result (`src/resident.ts`, the `ask` tool's refusal branch), peer card skill ids returned by the `roster` tool, and the VERIFIER's `verification.note` on `task_read`, which sat in the unbounded `meta` object while the same file put the identical field inside `wrapTaskText` on its other path. **All three FIXED 2026-08-30.** They were peer-chosen strings on the prompt path outside the 9.6 boundary renderer, which is the one place the neutralizer is wired. Measured on the refusal path with the exact template the resident uses: a `detail` carrying U+202E, U+200B, U+0007 and a literal `</room-message>` reaches the model with all four intact AND with no boundary at all, so that path violates item 3 as well as item 11 |
| 14 item 12 | Self-reports are decoration | **PARTIAL, first documented 2026-08-30.** The second half holds for the three decisions RFA-0.6 sect. 5.5 names, which are the ones that matter for safety: task state does not move on a peer's own `complete` when evidence is required (`verification.pending` is set instead), only an authorized verifier changes it (10.4), only a human-origin principal satisfies an approval, and spend is accounted from `runs.db` rather than from anything a member said. It did NOT hold in general until 2026-08-30, and the exception was this repository's own reliability gate rather than the hub: the eval harness keyed evaluator scores, the judge score and the review-queue write on the subject's SELF-REPORTED `run_id`, so a wrong or stale id attached a bad answer to another run or to none (an `UPDATE ... WHERE id = ?` matching nothing is not an error) and it never reached a human; and a subject answering `kind: "refuse"` dropped its own trial out of pass^k, which let a member lower the denominator of its own gate by declaring its state. **FIXED 2026-08-30** in `src/evals/runresolve.ts`: a claimed id is now a claim to be CHECKED against the store the write targets, the window resolves the run when the claim does not, an ambiguous or unreadable one is REPORTED rather than written, and a refusal leaves pass^k alone only when the subject's own run rows corroborate it - a subject that completed a turn and then declared a refusal now counts as a trial. Verified live: 40 evaluator feedback rows from one gate run, zero orphans. The residual is stated in that module: a refusal with no run row at all is indistinguishable from an honest pre-flight refusal, and closing it needs a hub-stamped correlation id, which is a wire change. The FIRST half is met since 2026-08-30 too: a resident composes `{run_id, cost_usd, num_turns, day_spend_usd}` into its own message body, and the CLI, the dashboard and the console used to print those numbers in the same style as the hub-measured figures beside them. All three now mark them, from one constant (`REPORTED_MARK` in `src/cli/ui.ts`; `console/index.html` cannot import it and repeats the word with a comment pointing there), and `rfa ask --json` renames the fields `cost_usd_reported` and `run_id_reported` so a script cannot mistake them either. A test asserts all three surfaces and fails if any drops the mark. Peer TEXT was already fine - the 9.6 boundary carries author, origin and home |
| 9.1, 14 item 7 | The unread cap's compaction summary | **SHIPPED 2026-08-30**, by the delivery the same day's amendment chose. The cap is enforced at its default of 200 and the `room_listen` result now carries `compaction` `{dropped, from_seq, to_seq, cap}` beside `compacted`, so a client learns which events it lost and under what policy rather than only how many. An in-stream `system` marker was built FIRST and reverted: measured, a client that persists its cursor and listens again accumulates it mid-array and `verifyChain` reports a second divergence carrying a FALSE tamper report against an innocent event (1 became 2), and 9.6, sect. 13 and INTEROP.md all say a verifier handles exactly two deviations. The reason is written into `src/store.ts` where the helper was, and `test/leases.test.ts` pins both halves: the summary is delivered, and no unchained event sits mid-stream |
| 13 | Hash-chain verification | **SHIPPED 2026-08-19.** Appendix F had no row for this at all: the hub has computed the chain since 0.1.7 but only ever FORWARD (stamp `prev_hash`, advance the head), so nothing walked a log backwards to check the links except two test files carrying private copies of the loop. `src/chain.ts` is now the one implementation and `scripts/verify-log.ts` an offline CLI (`npm run verify-log`) that reads files and never constructs a hub, as "verifiable offline" requires. Measured over the live 3,922-event room: **3,652 of 3,652 links intact with only `wrapped` removed**, which also confirms the 2026-08-18 stamp-before-hashing fix end to end. A log with no chain reports NOT-CHAINED rather than intact (ten of thirteen live logs predate the chain, and a green light over an unchecked log is worse than none). The scope qualifier lives in the library as a constant so no surface can drop it. `content_hash` has ONE producer since 2026-08-28 and it is not `redact`: the per-reader grant redaction of 10.3 item 7 stamps it in `eventForReader` (`src/store.ts`) on the task events it changes, so a member verifying the stream it RECEIVED still has the appended form's hash. `linkOf` therefore keys on the field's presence, not on `redacted: true`. 12.1's redaction remains the OTHER producer and is still unimplemented, so nothing on disk carries the field: it is stamped on the way out only, and `rfa log verify` will not meet one until `redact` ships. |
| 15 | `bad_request` | **SHIPPED 2026-08-30.** Failures raised inside the hub map to `bad_request`, and ARGUMENT validation now does too. The MCP SDK validated each tool's `inputSchema` before the handler ran and answered a violation with `Input validation error: Invalid arguments for tool room_listen: ...` - plain prose, no code, no envelope - which is the first error class a new implementer meets and exactly what this section's MUST is about. `advertised()` in `src/hub.ts` hands the SDK a proxy of the same zod object with only `~standard.validate` replaced, so `tools/list` publishes a BYTE-IDENTICAL schema (asserted in `test/hub.test.ts`) while enforcement moves into `run`, which throws `RfaError("bad_request")` and renders it in the same envelope every other refusal uses. The parsed value is assigned back onto the handler's arguments and stripped keys removed, so a handler sees exactly what the SDK used to hand it. Measured on all three classes: a missing required argument, a violated bound, and a bad enum |
...
| 16.1 | The amended core profile as a whole | **SHIPPED 2026-08-30**, and this row was wrong twice before it got here, which is worth more than the status. It first blamed 4.3's parked admission half, which sect. 16.1's own table says can never block a guestless hub; correcting it named three blockers; and attacking the claim that those three were fixed found that one fix (an in-stream compaction marker) broke chain verification and that a fourth blocker, sect. 15's argument-validation MUST, had been sitting in its own row unconnected to the profile all along. All of it now holds: `home`, `wrapped`, the `since` clamp, 9.3's amended `mentions` filter, security items 11 (classes AND application), 12 (both halves), and 13, 9.1's compaction summary, and sect. 15's `bad_request` on argument validation. It binds on the reference implementation from hub release 0.7.0 and this is 0.6.4, so it is met AHEAD of the deadline rather than at it. The lesson the row carries forward: a marker here is where a code check starts, never where it ends, and after a fix, attack the claim that it is fixed |
