...
| 14 item 12 | Self-reports are decoration | **PARTIAL, first documented 2026-08-30.** The second half holds for the three decisions RFA-0.6 sect. 5.5 names, which are the ones that matter for safety: task state does not move on a peer's own `complete` when evidence is required (`verification.pending` is set instead), only an authorized verifier changes it (10.4), only a human-origin principal satisfies an approval, and spend is accounted from `runs.db` rather than from anything a member said. **It does NOT hold in general**, and the exception is this repository's own reliability gate rather than the hub: the eval harness keys evaluator scores and the review-queue write on the subject's SELF-REPORTED `run_id`, so a wrong or stale id attaches a bad answer to another run or to none (an `UPDATE ... WHERE id = ?` matching nothing is not an error) and it never reaches a human; and a subject answering `kind: "refuse"` drops its own trial out of pass^k, which lets a member lower the denominator of its own gate by declaring its state. That is a harness defect, not a wire one, and it is recorded here because this row is where a reader will look for it. The FIRST half is not met either: a resident composes `{run_id, cost_usd, num_turns, day_spend_usd}` into its own message body and the CLI, the dashboard and the console print those numbers in the same style as hub-measured figures beside them, so an operator cannot tell which is which. Peer TEXT is fine - the 9.6 boundary carries author, origin and home. Closing it is a rendering change on three surfaces, not a protocol change |
