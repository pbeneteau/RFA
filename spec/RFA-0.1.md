...
| 8 | Chain ext `io.github.pbeneteau/chain` and the `would_deadlock` refusal (0.1.9; RFA-0.8 sect. 2.2) | **PENDING.** Nothing mints, propagates, or refuses on chain ids at HEAD; builds with RFA-0.8 rung 2 |
| 8 | Hub-defaulted `reply_by` on cross-home requests (0.1.9; RFA-0.8 sect. 2.3) | **PENDING.** The hub stamps no default `reply_by` at HEAD; builds with RFA-0.8 rung 2 |
...
| 10.3 | `resources[]` on claim: the authority-segment grammar, NFC validation, the 256-byte/16-key bounds, prefix-or-equal segment intersection, refuse-never-wait (0.1.9; RFA-0.8 sect. 2.1 items 1-5) | **PENDING.** The claim path reads no `resources[]` argument at HEAD; builds with RFA-0.8 rung 7 |
| 10.3 | Widening as a fresh mini-claim and the creator-approved reservation after 3 refused widenings (0.1.9; RFA-0.8 sect. 2.1 item 6) | **PENDING.** Builds with RFA-0.8 rung 7 |
| 10.3 | Grants persisted on the task, lifetime bound to the claim's, never outliving the task (0.1.9; RFA-0.8 sect. 2.1 item 7) | **PENDING.** No grant store exists at HEAD; a process-local grant map is non-conformant by the same item; builds with RFA-0.8 rung 7 |
| 10.3 | The opaque keyed digest (`hmac-sha256:`) in `task_conflict` for a non-local claimant blocked by a `local/...` key (0.1.9; RFA-0.8 sect. 2.1 item 8) | **PENDING.** Builds with RFA-0.8 rung 7 |
...
| 15 | `lease_expired` thrown with its `{current_attempt, current_owner, task_state}` data (0.1.9 commitment; RFA-0.8 sect. 2.4) | **PENDING.** A stale claim token still surfaces as `unauthorized` (the same defect the 10.3 claim-leases row records). 0.1.9 changed no text here: the error and its `data` have been defined since 0.1.8, and RFA-0.8's acceptance commits the reference hub to throwing them |
