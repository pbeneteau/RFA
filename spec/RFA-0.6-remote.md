...
| Requirement | Status |
|---|---|
| `deploy/peers.json`, `peer_id`, invites | Not implemented, and PARKED on two gates (a named counterparty AND a reversal of the 2026-08-19 privacy decision; sect. 8.9). **`home` is the exception and IS implemented**: hub-derived, defaulting to `"local"`, persisted, on every roster record and envelope `from`, and load-bearing in the verifier rule and the approval-registration check. The vocabulary was frozen early on purpose. |
