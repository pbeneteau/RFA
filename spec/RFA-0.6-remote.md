...
- ~~**Not implemented.** No transport credential exists in the reference hub today, on either side.~~ **SHIPPED (v0.6.0a, 2026-08-18).** `RFA_MCP_TOKENS` / `--mcp-token` on the hub, `RFA_TOKEN` read per call on the client, and `/mcp` deliberately unlockable (a per-source lockout is global behind a loopback-terminating proxy, which took every resident offline once). Re-verified live 2026-08-20: unauthenticated `/mcp` returns 401 while `GET /healthz` returns 200.
...
- Per-human credential handling was in scope for the same rung and is not remote-specific. **SHIPPED 2026-08-19** (`src/principals.ts`): `principal_id`, one constant-time compare shared by `POST /auth` and the WIRE join path, per-principal console memberships with exact-name matching, and `refs.principal` on every human-origin intervention. Two corrections to this bullet's own diagnosis: `src/main.ts` had already been fixed and the surviving non-constant-time `includes` was in `src/store.ts`'s join path; and `consoleMembership()` already matched on an exact name rather than `startsWith`. It also fixed a latent over-refusal in spec 10.4's same-principal rule, which compared `home` and so treated two different humans on one hub as the same party. The grant matrix stays parked.
...
6. **A per-framework timeout table.** The table MUST exist and every row MUST stamp the era and the client version it was measured against, and every row MUST say whether a re-runnable script produced it, because that is the difference between a measurement and a recollection. **No numbers are stated normatively here**, and that remains true even though sect. 13's committed script now exists for one client: the table's home is `INTEROP.md`, which the operator publishes and revises per hub, not this document, which would date the moment a client ships a new version. The first committed row (2026-09-08, `codex-cli 0.153.4`, modern era) is that **the OpenAI Codex CLI does not cancel a `room_listen` at 25, 45 or 60 seconds**, hermetically and under an operator's own configuration; 60 000 ms is the hub's own ceiling on `timeout_ms`, so it establishes "at least the longest poll RFA can ask for" and NOT that client's limit, which needs a deliberately slow server to find. It does not bear on the row below it: the Agents SDK is a different client. Two rows were reported in W4 sect. 3 as **operator report, uncommitted** (an OpenAI Agents SDK client-session timeout defaulting to a few seconds, so a 20-second `room_listen` dies with a timeout error that reads to a stranger as "RFA is broken"; and `langchain-mcp-adapters` surviving a long poll only because the hub answers a legacy-era client with SSE framing governed by a different timeout). The second observation carries its own warning wherever it is written down: it is **an accident of the dual-era handler and MUST NOT be published as a per-framework contract**, and its coincidental agreement with item 3's recommended ceiling has no shared cause.
...
- **Retention and exit.** The room's retention window MUST be stated in the join contract, and spec 11.3 now names where: the `instructions` string, alongside the plaintext-by-design statement. No new join-contract field is added for it. A leaving peer MUST be able to export the events it sent and received in one command; the verifiable-log export (`npm run verify-log`, **shipped 2026-08-19**, sect. 12) shares the same code path; the per-member export half is still to come.
...
**DECIDED 2026-08-19: the repository stays private.** So none of the three obligations above will exist, and this section's precondition therefore **cannot be met**: admitting a peer outside the operator's organization is parked by decision and not only by the absence of a counterparty. The three artifacts are not themselves what privacy forbids (a private repository can carry a `SECURITY.md` and run CI); what privacy settles is this section's opening premise, that an organization cannot reasonably be told to point its agents at a hub whose implementation it cannot read. Everything in v0.6 that does not touch an external peer stays in scope and is listed as such in the merged ladder.
...
| **v0.6.2** | The gate over mutating task actions with the versioned check-input contract of sect. 7.2 and its hold-to-refuse degradation; task field size caps; the widened sanitizer set; `evidence_required` forced at claim; the verifier rule with capped rejections; the structured approval ext **with `src/bridge.ts` updated in the same change**; console guest rendering items 1 to 3 | 5.7, 7.2, 7.4, 7.6 | **SHIPPED 2026-08-19.** All of it, verified live against the shipped `deploy/gate.json` and in a real browser. Two corrections to this document's own predictions: the approval-ext change broke FIVE producers, not the one named in sect. 7.4; and `evidence_required`-at-claim plus the capped verifier rule had already landed with v0.6.1. Console items 4 to 6 (per-peer spend, lease state, self-report styling) still move with the admission half, though item 5 is locally useful and needs no peer |
...
| **v0.6.3b** (audit artifact and release) | `room_admin redact`; retention in the join contract and per-member export; `verify-log`; `/healthz`; Docker and compose; `SECURITY.md`, supported versions, CI | 7.7, 8.5-8.9 | **SPLIT by the privacy decision of 2026-08-19 (sect. 8.9).** Scheduled and buildable: redaction, retention plus disclosure, `verify-log`, `/healthz`. **Cancelled**: `SECURITY.md`, supported versions, CI. **Still deferred on its own reason**: Docker and compose, on the exposure posture of sect. 4.5 |
| **v0.6.4** | Per-human `principal_id` with constant-time comparison and per-principal console memberships; retrieval set and gate verdict recorded per answer (forensics only, no detector claim). Then, with the admission half: `usd_per_day` enforced per sect. 7.1 and `peer_rpm` per `(peer_id, recipient home)` | 4.4, 7.1 | **The per-human half SHIPPED 2026-08-19**: `principal_id`, the constant-time compare moved to `src/principals.ts` and shared with the WIRE join path (the surviving `includes` was there, not in `main.ts` as sect. 4.4 says), per-principal console memberships, and `refs.principal` on every human-origin intervention. It also fixed a latent over-refusal: 10.4's same-principal rule compared `home`, so two different humans on one hub could not verify each other's work. **The forensics half also SHIPPED 2026-08-20**: the retrieval set (which knowledge files an answer opened, in order, with greps and their patterns) on every run, and the gate verdict on the hub's own span. The latter needed an enabling fix, since no OTel context manager was registered so `getActiveSpan` from inside `send` returned nothing. Remaining: only the two per-peer budgets, which stay with the admission half |
...
| Requirement | Status |
|---|---|
| `deploy/peers.json`, `peer_id`, invites | Not implemented. The parking was re-ruled 2026-08-21: a counterparty was named (the owner's cross-account Claude session joined the live room the same day, and a LangChain peer is expected), so the remote MECHANICS un-parked; the admission HARDENING here still waits for the first peer the operator does not control, or a release. **Two exceptions ARE implemented**: `home` (hub-derived, defaulting to `"local"`, persisted, on every roster record and envelope `from`, load-bearing in the verifier rule and the approval-registration check), and the first admission slice, bearer-implied join (2026-08-21: `join_bearer_sha256` room policy admits a listed transport bearer with no `join_secret`, the hash arriving on request context, never as an argument; per-peer revocation when each peer holds its own bearer). |
...
| Transport authentication on `/mcp`; credentialed `room_create` | **SHIPPED (v0.6.0a)**: `mcpAuthorized` accepts an operator token or a live workbench session token, `/mcp` is deliberately unlockable (a per-source lock is global behind a loopback-terminating proxy), and the client reads `RFA_TOKEN` per call. Re-verified live 2026-08-19: unauthenticated `/mcp` returns 401 while `/healthz` returns 200. |
...
| `npm run verify-log` | **SHIPPED 2026-08-19**: `src/chain.ts` is the one implementation of the construction (two test files each carried a private copy) and `scripts/verify-log.ts` is an offline CLI that reads files and never constructs a hub. Measured over the live room: 3,652 of 3,652 links intact with only `wrapped` removed. A log carrying no chain reports NOT-CHAINED, never intact. |
| Claim lease, release, `claim_token`, `lease_expired` | **SHIPPED (v0.6.1)**: `attempt`, `lease_expires`, `released_at`, `max_attempts`, a `release` action, automatic release on offline/leave/eviction with `task_released`, and `claim_token` in the claim RESULT only. Verified by reproducing an outside integrator's wedge. Still pending: re-binding a claim from a new member id (needs per-member principals). |
| Gate over task actions | **SHIPPED (v0.6.2, 2026-08-19)**: a second call site with the versioned check input of sect. 7.2, hold-degrades-to-refuse, and task text size-capped. Verified live against the shipped `deploy/gate.json`: a private key in a task DESCRIPTION is now refused where it previously created without comment. |
| `since` clamped to the join point | **SHIPPED**: `joinSeq` is a persisted member field and `visibleSince` clamps, called from BOTH `listen` and `watch` (a clamp on one path only would be no clamp). The stricter policy applies it to every member, not just non-local ones. |
| Verifier authorization | **SHIPPED (v0.6.1)**: a verifier must be a local member, the creator, or a human principal; `verifier_home` recorded; rejections capped per `(task, attempt)` and surviving a bounce through `complete`. The same-principal test is live for HUMAN principals (`src/store.ts`, `samePrincipal`, with a conservative home fallback); agents carry no principal, so an agent party's second shared-secret membership still self-verifies until sect. 4.3 ships. |
...
| Restart-durable send idempotency | **SHIPPED (v0.6.3a)**: a replayed `message_id` returns its original result with `replayed: true` and empty recipients, rather than fabricating dispositions. |
| Atomic snapshot write | **SHIPPED (v0.6.3a)**: temp file plus rename, `fsync` of BOTH the file and its directory, and rebuild-from-log when a snapshot will not parse (an unreadable snapshot used to make a room silently unreachable while its log sat on disk). |
| Shared neutralizer on the prompt path | **SHIPPED**, and widened 2026-08-19: `wrapForModel` and `sanitizeForMemory` share one `neutralize` implementing sect. 14.11's four MUST classes, plus the TAG block SHOULD, plus a `room-task` boundary for task text (which reached the model prompt with NO boundary while a message from the same author had one). Whitespace folding is implemented but SCOPED to the length-capped approval preview, deliberately not the model boundary; read that as a narrowing, not full implementation. |
| Store lock with `O_EXCL` and heartbeat | **SHIPPED (v0.6.3a)**: `openSync(lock, "wx")` is the O_EXCL acquire, with a 10s heartbeat and a 60s staleness window, because PID liveness means nothing across containers. Now also the source of `/healthz`'s store predicate. |
| `db.backup()` instead of the CLI shell-out | **SHIPPED**: `better-sqlite3`'s own backup, with the plain copy kept as a genuinely last-resort fallback. The driver is already a dependency, the `sqlite3` CLI is not guaranteed to exist, and shelling out put the destination path through a shell-quoted string. |
| Extensions advertised in `server/discover` | **SHIPPED**: the `rfa=` capability string carries `spec_version`, `profiles` and `extensions` (`io.github.pbeneteau/rooms`, `io.github.pbeneteau/approval`). |
...
**Amendment (2026-09-08)** - sect. 6.1 item 6 gains one requirement and loses one excuse. Every row of the timeout table MUST now say whether a re-runnable script produced it, because "operator report" and "measured" are different grades of evidence and the table read as though they were not. And the item no longer says no committed script exists: one does, for the Codex CLI, and its result is recorded here as the first committed row. The table itself still lives in `INTEROP.md` rather than in this document, which is now stated as the reason no numbers appear here rather than left implicit in sect. 13's rule.
