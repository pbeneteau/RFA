...
| Verifier authorization | **SHIPPED (v0.6.1)**: a verifier must be a local member, the creator, or a human principal; `verifier_home` recorded; rejections capped per `(task, attempt)` and surviving a bounce through `complete`. Pending: the same-principal test across two memberships, which needs the principals of sect. 4.3. |
