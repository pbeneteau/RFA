...
- Verify changes with `npm test` (it prints the live count; the script globs `test/*.test.ts`, so a new file counts immediately, and no document states the number because a stated count rotted here twice) and `npm run e2e` (wire scenarios, writes `reports/latest.md`). Brain or knowledge changes also need `npx tsx dogfood/parity.ts`, and run it TWICE: a single pass has hidden a real regression here.
